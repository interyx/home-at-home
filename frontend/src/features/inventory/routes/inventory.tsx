import { Card, CardBody, CardFooter, Divider, Image, Table, TableBody, TableHeader, TableColumn, TableRow, TableCell, getKeyValue, Button, ButtonGroup, Modal, ModalContent, ModalHeader, ModalBody, useDisclosure, Input, ModalFooter } from "@nextui-org/react";
...
export default function Inventory() {
  ...
  return (
    <div className="h-100 min-h-dvh bg-background text-foreground mx-2 pb-6 p-0">
      <h1 className="text-5xl text-center text-orange-300 font-prata mt-4">Living Room</h1>
      {/* container component */}
      ...
      <div className="lg:grid lg:grid-cols-3 mt-4">
        ...
        <div className="grid lg:col-start-2 grid-cols-3 lg:grid-cols-4 gap-2 lg:gap-4 border border-slate-600 p-3 lg:p-5">
          {containers[0].containers.map((item, idx) => {
            return (
        ...
        <Table aria-label="Items table" className="lg:col-start-2">
          ...
          <TableBody items={containers[0].items}>
            {(item) => (
